<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Input Perizinan Siswa') }}</h2>
            ...
            <form action="{{ route('perizinan.store') }}" method="POST"
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" id="keterangan" rows="2" placeholder="Alasan sakit/izin (opsional)"
                            class="w-full rounded-md border-gray-300 shadow-sm text-sm">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Lampiran Bukti <span class="text-red-500">*</span>
                        </label>
                        <p class="text-xs text-gray-400 mb-2">Paste gambar (Ctrl+V) dari clipboard sebagai bukti
                            perizinan.</p>
                        <div id="lampiran-dropzone"
                            ...
                            <div id="lampiran-placeholder">
                                ...
                                <p class="text-sm text-gray-400">Ctrl+V untuk paste gambar</p>
                                <p class="text-xs text-gray-300 mt-1">atau klik area ini lalu paste</p>
                            </div>
                            <img id="lampiran-preview" src="" alt="Preview"
                                class="hidden max-h-48 rounded-lg object-contain" />
                            <button type="button" id="lampiran-remove"
                                class="hidden absolute top-2 right-2 w-7 h-7 rounded-full bg-red-500 text-white text-xs flex items-center justify-center hover:bg-red-600 shadow"
                                title="Hapus gambar">&times;</button>
                        </div>
                        <input type="hidden" name="lampiran" id="lampiran-input" value="{{ old('lampiran') }}">
                        @error('lampiran')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
                    <p class="text-xs text-gray-500">
                        Lampiran wajib diisi. Perizinan otomatis diterapkan ke jurnal.
                    </p>
                    <button type="submit"
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // Focus hint: click on dropzone focuses it for paste
