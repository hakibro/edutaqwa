<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Edit CP') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('cp.update', $cp) }}" method="POST">
                        @csrf @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="mapel_id"
                                ...
                                @foreach ($mapels as $m)
                                    <option value="{{ $m->id }}"
                                        {{ old('mapel_id', $cp->mapel_id) == $m->id ? 'selected' : '' }}>
                                        {{ $m->nama }}</option>
                                @endforeach
                            </select>
                            @error('mapel_id')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="kode" value="{{ old('kode', $cp->kode) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" rows="5"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>{{ old('deskripsi', $cp->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
