<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Input Presensi') }}</h2>
        ...
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            @if ($existingToday)
                <div class="mb-4 rounded-md bg-yellow-50 p-4 text-sm text-yellow-800">
                    Presensi untuk jadwal ini hari ini sudah ada. Silakan gunakan menu Edit untuk mengubah.
                </div>
            @endif
                            ...
                            @foreach ($siswas as $i => $s)
                                <div
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <span class="ml-2 text-xs text-gray-400">{{ $s->nis }}</span>
                                    </div>
                                    <input type="hidden" name="siswa[{{ $i }}][id]"
                                        value="{{ $s->id }}">
                                    <select name="siswa[{{ $i }}][status]"
                                        class="rounded-md border-gray-300 text-sm py-1
                                        {{ old("siswa.{$i}.status", 'hadir') === 'hadir' ? 'bg-green-50 border-green-300' : '' }}
                                        {{ old("siswa.{$i}.status") === 'alpha' ? 'bg-red-50 border-red-300' : '' }}
                                        {{ old("siswa.{$i}.status") === 'sakit' ? 'bg-yellow-50 border-yellow-300' : '' }}
                                        {{ old("siswa.{$i}.status") === 'izin' ? 'bg-orange-50 border-orange-300' : '' }}
                                        {{ old("siswa.{$i}.status") === 'terlambat' ? 'bg-purple-50 border-purple-300' : '' }}"
                                        ...
                                        <option value="hadir"
                                            {{ old("siswa.{$i}.status", 'hadir') === 'hadir' ? 'selected' : '' }}>Hadir
                                        </option>
                                        <option value="sakit"
                                            {{ old("siswa.{$i}.status") === 'sakit' ? 'selected' : '' }}>Sakit</option>
                                        <option value="izin"
                                            {{ old("siswa.{$i}.status") === 'izin' ? 'selected' : '' }}>Izin</option>
                                        <option value="alpha"
                                            {{ old("siswa.{$i}.status") === 'alpha' ? 'selected' : '' }}>Alpha</option>
                                        <option value="terlambat"
                                            {{ old("siswa.{$i}.status") === 'terlambat' ? 'selected' : '' }}>Terlambat
                                        </option>
                                    </select>
                                    <input type="text" name="siswa[{{ $i }}][keterangan]"
                                        placeholder="Ket." value="{{ old("siswa.{$i}.keterangan") }}"
