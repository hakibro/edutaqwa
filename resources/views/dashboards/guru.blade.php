@php
    ...
    if ($guru) {
        ...
        $jamKerjaHariIni = \App\Models\JamKerjaLembaga::where('lembaga_id', $lembagaId)
            ->where('hari', $todayName)
...
<x-app-layout>
    <div x-data="dashboardGuru()" class="relative overflow-x-clip">
        ...
        <main class="px-4 pt-2 space-y-4 max-w-lg mx-auto content-safe-bottom relative">
            ...
            {{-- Warning absensi PTK --}}
            @if ($guru && $guru->isStruktural() && (!$absensiHariIni || !$absensiHariIni->check_in))
                <div class="rounded-2xl bg-amber-50 border border-amber-300/70 p-4 flex items-start gap-3 shadow-sm">
                    <div class="w-9 h-9 rounded-full bg-amber-200 flex items-center justify-center shrink-0 mt-0.5">
                        <svg class="w-5 h-5 text-amber-700" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-amber-800">Anda belum melakukan absensi hari ini.</p>
                        <p class="text-xs text-amber-600 mt-0.5">Sebagai PTK, wajib melakukan check-in sebelum memulai
                            aktivitas.</p>
                        <a href="{{ route('absensi-ptk.index') }}"
                            class="inline-flex items-center gap-1 mt-2 rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm active:scale-[0.98] transition">
                            Absen Sekarang
            ...
            {{-- Warning belum check-out — jam kerja sudah selesai --}}
            @php
                $blmCheckout =
                    $guru &&
                    $guru->isStruktural() &&
                    $absensiHariIni &&
                    $absensiHariIni->check_in &&
                    !$absensiHariIni->check_out &&
                    $jamKerjaHariIni &&
                    now()->format('H:i') > $jamKerjaHariIni->jam_pulang;
            @endphp
            @if ($blmCheckout)
                <div class="rounded-2xl bg-red-50 border border-red-300/70 p-4 flex items-start gap-3 shadow-sm">
                    <div class="w-9 h-9 rounded-full bg-red-200 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-red-800">Anda belum check-out hari ini.</p>
                        <p class="text-xs text-red-600 mt-0.5">Jam pulang {{ $jamKerjaHariIni->jam_pulang }} sudah
                            lewat. Segera lakukan check-out.</p>
                        <a href="{{ route('absensi-ptk.index') }}"
                            class="inline-flex items-center gap-1 mt-2 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm active:scale-[0.98] transition">
                            Check-Out Sekarang
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
