    ...
    <div class="py-12">
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <form action="{{ route('tahun-ajaran.update', $tahunAjaran) }}" method="POST" class="p-6 space-y-4">
                    @csrf @method('PUT')
                    ...
                    @if (auth()->user()->isSuperAdmin())
                        <div>
                            ...
                            <select id="yayasan_id" name="yayasan_id"
                                ...
                                disabled>
                                <option value="{{ $tahunAjaran->yayasan_id }}">{{ $tahunAjaran->yayasan->nama }}
                                </option>
                            </select>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <x-text-input id="nama" name="nama" type="text" class="mt-1 block w-full"
                            :value="old('nama', $tahunAjaran->nama)" placeholder="2025/2026" required />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="tanggal_mulai" name="tanggal_mulai" type="date"
                                class="mt-1 block w-full" :value="old('tanggal_mulai', $tahunAjaran->tanggal_mulai->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="tanggal_selesai" name="tanggal_selesai" type="date"
                                class="mt-1 block w-full" :value="old('tanggal_selesai', $tahunAjaran->tanggal_selesai->format('Y-m-d'))" required />
