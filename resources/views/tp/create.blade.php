<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Tambah TP') }}</h2>
    </x-slot>
                    ...
                    <form action="{{ route('tp.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="cp_id"
                                ...
                                @foreach ($cps as $c)
                                    <option value="{{ $c->id }}"
                                        {{ old('cp_id', request('cp_id')) == $c->id ? 'selected' : '' }}>
                                        {{ $c->kode ?? 'CP' }} — {{ $c->mapel->nama }} (Fase {{ $c->fase }})
                                    </option>
                                @endforeach
                            </select>
                            @error('cp_id')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="kode" value="{{ old('kode') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                placeholder="Contoh: TP.MAT.E.1.1">
                        </div>
