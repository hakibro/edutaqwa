<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Edit Kelas') }}: {{ $kelas->nama }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <form action="{{ route('kelas.update', $kelas) }}" method="POST" class="p-6 space-y-4">
                    @csrf @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="lembaga_id" name="lembaga_id"
                                ...
                                @foreach ($lembagas as $l)
                                    <option value="{{ $l->id }}" @selected(old('lembaga_id', $kelas->lembaga_id) == $l->id)>{{ $l->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="tingkat" name="tingkat"
                                ...
                                @foreach (['X', 'XI', 'XII', 'XIII', 'VII', 'VIII', 'IX', '1', '2', '3', '4', '5', '6'] as $t)
                                    <option value="{{ $t }}" @selected(old('tingkat', $kelas->tingkat) == $t)>{{ $t }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="jurusan_id" name="jurusan_id"
                                ...
                                @foreach ($jurusans as $j)
                                    <option value="{{ $j->id }}" @selected(old('jurusan_id', $kelas->jurusan_id) == $j->id)>{{ $j->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
