<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Tambah ATP') }}</h2>
    </x-slot>
                    ...
                    <form action="{{ route('atp.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tp_id"
                                ...
                                @foreach ($tps as $t)
                                    <option value="{{ $t->id }}"
                                        {{ old('tp_id', request('tp_id')) == $t->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="minggu_ke" value="{{ old('minggu_ke') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                min="1" required>
                            @error('minggu_ke')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
