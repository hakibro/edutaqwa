<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Edit Mapel') }}</h2>
    </x-slot>
                ...
                <div class="p-6">
                    <form action="{{ route('mapel.update', $mapel) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="lembaga_id"
                                    ...
                                    @foreach ($lembagas as $l)
                                        <option value="{{ $l->id }}"
                                            {{ old('lembaga_id', $mapel->lembaga_id) == $l->id ? 'selected' : '' }}>
                                            {{ $l->nama }}</option>
                                    @endforeach
                                </select>
                                @error('lembaga_id')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="kelompok_mapel_id"
                                ...
                                @foreach ($kelompokMapels as $km)
                                    <option value="{{ $km->id }}"
                                        {{ old('kelompok_mapel_id', $mapel->kelompok_mapel_id) == $km->id ? 'selected' : '' }}>
                                        {{ $km->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $mapel->nama) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                            @error('nama')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('deskripsi', $mapel->deskripsi) }}</textarea>
                        </div>
